...
func LaunchBootstrapper(newProcess bool, robloxArgs string) {
	...
	Println("allocating new instance")
...
func CheckAndPerformUpdates(installDir string, latestVersion clientsettingscdnResponse) {
...
func RequiresInstallation(latestVersion string) bool {
	...
	for _, v := range versions {
		if v.Name() == latestVersion && v.IsDir() {
			...
			err := os.RemoveAll(filepath.Join(common.LPath("./versions"), v.Name()))
			if err != nil {
